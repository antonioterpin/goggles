...
| `GOGGLES_HOST_LOG` | unset | Path to capture the dedicated host's stdout/stderr. The host outlives the process that spawns it, so its output is discarded by default rather than written to that process's (eventually closed) terminal; set this to a file to capture it when debugging. |
