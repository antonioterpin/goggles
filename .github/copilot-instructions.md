...
- **Transitional Structure**: The project is currently migrating to a new API.
	- `goggles/api.py` is the official, up-to-date API entrypoint. All new features and integrations are exposed here.
	- `goggles/_core/` and `goggles/history/` contain the main implementations for logging, device history, and core logic.
	- **Legacy code**: Everything in the top-level `goggles/` folder except `__init__.py` and `api.py` is considered legacy. Avoid extending or modifying these unless porting features to the new API.
...
## Tips for AI Agents
- Use only `api.py` for new features and integrations; legacy code is for reference/porting only.
